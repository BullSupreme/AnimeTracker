...
{season_opts_html}        </select>
    ...
    // ============================================================
    // 9ANIME SEARCH CLEANUP
    // ============================================================
    function sanitize9AnimeSearchTitle(title) {{
        if (!title) return '';

        let searchTitle = String(title).trim().replace(/\\s+/g, ' ');
        const trailingSymbols = "!\\"#$%&'*+,./:;<=>?@^_`{{|}}~-";
        const stripTrailingSymbols = value => {{
            let result = value.trim();
            while (result) {{
                const last = result[result.length - 1];
                if (last.trim() && !trailingSymbols.includes(last)) break;
                result = result.slice(0, -1).trimEnd();
            }}
            return result.trim();
        }};

        searchTitle = stripTrailingSymbols(searchTitle);
        searchTitle = searchTitle.replace(/\\s*-\\s*(?:kun|chan|san|sama|senpai|sensei|dono)$/i, '').trim();
        searchTitle = stripTrailingSymbols(searchTitle);
        searchTitle = searchTitle.replace(/(?:^|\\s)(?:s|season|series|part|cour)\\d+(?:st|nd|rd|th)?$/i, '').trim();
        searchTitle = searchTitle.replace(/(?<=[a-z])\\d+(?:st|nd|rd|th)?$/i, '').trim();

        return stripTrailingSymbols(searchTitle);
    }}

    function create9AnimeSearchUrl(title) {{
        const searchTitle = sanitize9AnimeSearchTitle(title);
        if (!searchTitle) return 'https://9anime.me.uk/';
        const nineAnimeQuery = new URLSearchParams({{ s: searchTitle }}).toString();
        return `https://9anime.me.uk/?${{nineAnimeQuery}}`;
    }}
    ...
    document.getElementById('ctx-9anime').addEventListener('click', () => {{
        if (contextTarget) {{
            ...
            window.open(create9AnimeSearchUrl(eng), '_blank');
        }}
        document.getElementById('context-menu').style.display = 'none';
    }});
